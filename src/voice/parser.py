...
PART_PATTERNS = [
    ...
    # set_cue_time → None (агент объяснит что надо делать вручную на пульте)
    ("set_cue_time",  r"(?:установ\w*|устанавл\w*|set|поставь|измен[иь])\s*(?:время|time|врем)\s*(?:на\s*)?(?:kue|cue|куэ|кью)?\s*[-–]?\s*(\d+[.,]?\d*)\s*[-–=]?\s*(\d+[.,]?\d*)", None),
    ("set_cue_time2", r"(?:kue|cue|куэ|кью)\s*(\d+[.,]?\d*)\s*[-–=]?\s*(?:время|time|врем)\s*[-–=]?\s*(\d+[.,]?\d*)", None),
...
def parse(text):
    ...
    for name, pat, action in PART_PATTERNS:
        ...
        if m:
            if action is None:
                return None  # явный сигнал «отдать агенту»
